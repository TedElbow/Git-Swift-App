...
/// Holds the dependency container created at launch; LittleStreakUp reads it and passes into the view hierarchy.
final class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate, MessagingDelegate {
    ...
    /// Dependency container created at launch. Set in didFinishLaunching; LittleStreakUp reads it and injects into views.
    ...
    /// Delivers buffered push URLs after `DeepLinkRouter` is wired (see `LittleStreakUp` `onAppear`).
    @MainActor
